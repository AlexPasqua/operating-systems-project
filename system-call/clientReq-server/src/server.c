...
unsigned int usercode(const char *); //funz per creare una codice abbinato ad una stringa
...
void generate_key(struct Response *response, struct Request *client_data){
  /* genero la chiave:
   *  prendo il timestamp, accodo un codice della stringa dello user (4 cifre),
  ...
  srand(timestamp);
  response->key = ((timestamp * MILLION) + (usercode(client_data->user) * 100) +
             ((client_data->service[0] == 'i') ? 20 : ((client_data->service[1] == 't') ? 0 : 10)) +
             (rand() % 10)) % THOUSAND_BILLIONS;
}

//==============================================================================
unsigned int usercode(const char *str){
